...
def pagexmlcombine(ocrindex, gtindex, xmlfile):
    ...
    for r in textregions:
        ...
        if "path" not in coordmap[rid]:
            raise ValueError("Segment for region " + rid
                             + " not found in pagedir "+thispagedir+"!")
        ...
        for n, l in enumerate(sorted(glob(coordmap[rid]["path"]+"/*.coords"))):
            ...
            with open(l) as f:
                b = f.read().split(",")
            b = [int(x) for x in b]
            # boxes: [(x1, y1, x2, y2),...], cf. kraken segmenter
            b = [b[1], b[0], b[3], b[2]]
            ...
            coords = [(x[0] + coordmap[rid]["offset"][0],
                      x[1] + coordmap[rid]["offset"][1]) for x in coords]
    ...
    for rid in sorted(coordmap):
        textregion = root.find('.//ns:TextRegion[@id="'+rid+'"]',
                               namespaces=ns)
        regiontext = []
        ...
        for lid in coordmap[rid]["lines"]:
            linexml = textregion.find('./ns:TextLine[@id="'+lid+'"]',
                                      namespaces=ns)
            if linexml is None:
...
@click.command()
@click.option('--ocrindex', '-ocrx', default=1,
              help='Index attribute of the OCR text.')
@click.option('--gtindex', '-gtx', default=0,
              help='Index attribute of the ground truth text.')
@click.argument('xmlfiles', type=click.Path(exists=True, dir_okay=False,
                writable=True), nargs=-1)
def loopfiles(ocrindex, gtindex, xmlfiles):
    """
        Takes a bunch of PageXML files and integrates information retrieved
        from a subfolder called 'Pages' in the same directory.
    """
    if xmlfiles:
